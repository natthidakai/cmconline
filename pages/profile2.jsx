...
const Profile = () => {
    ...
    useEffect(() => {
        if (status === 'unauthenticated') {
            router.push('/signin'); // ถ้าไม่ได้ยืนยันตัวตน ให้เปลี่ยนเส้นทางไปยังหน้าล็อกอิน
        }
    }, [status]);

    if (status === 'loading') {
        return <p>กำลังโหลด...</p>; // สถานะกำลังโหลด (ถ้ามี)
    }

    if (!session) {
        return <p>กรุณาเข้าสู่ระบบเพื่อเข้าถึงหน้านี้</p>; // ข้อความแสดงหากยังไม่ได้เข้าสู่ระบบ
    }
